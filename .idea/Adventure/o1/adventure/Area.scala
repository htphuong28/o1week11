...
class Area(var name: String, var description: String):
  ...
  def addItem(item: Item): Unit =
    itemList.addOne(item.name, item)

  def contains(itemName: String): Boolean = itemList.contains(itemName)
  ...
  def fullDescription: String =
    val exitList =
      if itemList.isEmpty then s"\n\nExits available: " + this.neighbors.keys.mkString(" ")
      else s"\nYou see here: " + this.itemList.keys.mkString(" ") + "\n\nExits available: " + this.neighbors.keys.mkString(" ")
